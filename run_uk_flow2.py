...
def run():
    with sync_playwright() as pw:
        ...
        # Get first VISIBLE itinerary-details link
        links = page.locator("a[href*='itinerary-details']")
        print(f"  itinerary-details links (all): {links.count()}", flush=True)
        ...
        if visible_link is None:
            # Try a broader selector for visible cruise title links
            visible_link = page.locator("a[href*='itinerary-details']:visible").first
            if page.locator("a[href*='itinerary-details']:visible").count() == 0:
        ...
        detail_btns = page.evaluate(
            ...
            ".map(e=>e.innerText.trim()).filter(t=>t.length>0 && t.length<60).slice(0,20)"
        )
        print(f"  Visible btns/links: {detail_btns}", flush=True)
        ...
        for sel in ["button:has-text('Book Now')", "a:has-text('Book Now')",
            ...
            if loc.count() > 0 and loc.first.is_visible():
                ...
                break
        if not clicked_book:
            print("  No Book Now button found", flush=True)

        page.wait_for_load_state('domcontentloaded')
        page.wait_for_timeout(5000)
        print(f"Step 3 - After Book Now URL: {page.url}", flush=True)
        ...
        body3 = page.evaluate("() => document.body.innerText.slice(0, 1000)")
        print(f"  Body text: {body3[:500]}", flush=True)
        ...
        if 'guest-count' in current_url or ('guest' in current_url and 'count' in current_url):
            ...
            page.wait_for_load_state('domcontentloaded')
            page.wait_for_timeout(3000)
            print(f"Step 4 - After guest-count: {page.url}", flush=True)
        else:
            print(f"  Not on guest-count (URL: {page.url})", flush=True)
        ...
        if 'stateroom-type' in current_url:
            ...
            print(f"Step 5 - After stateroom-type: {page.url}", flush=True)
        else:
            print(f"  Not on stateroom-type page (URL: {page.url})", flush=True)
        ...
        if 'packages' in current_url:
            ...
            for txt in ["I'LL PAY MORE ONBOARD", "PAY MORE ONBOARD", "No Thanks", "Continue", "CONTINUE"]:
                ...
                if btn.count() > 0 and btn.first.is_visible():
                    ...
                    page.wait_for_timeout(2000)
                    break
            print(f"Step 6 - After packages: {page.url}", flush=True)
        else:
        ...
        page.wait_for_timeout(2000)
        current_url = page.url.lower()
        print(f"Step 7 - Current URL: {page.url}", flush=True)
        ...
        if 'stateroom-summary' in current_url:
            ...
            if cont.count() > 0 and cont.first.is_visible():
                ...
                page.wait_for_load_state('domcontentloaded')
                page.wait_for_timeout(3000)
                print(f"Step 8 - After CONTINUE: {page.url}", flush=True)
            ...
            print(f"  Not on stateroom-summary (URL: {page.url})", flush=True)
